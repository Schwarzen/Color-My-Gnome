...
class DynamicPopupWindow:
    """
    A reusable layout factory that creates an Adw.Window container,
    injects custom blueprints, and manages scaling behavior automatically.
    """

    @classmethod
    def spawn(cls, parent_window, title, content_widget, width=450, height=400):
        # 1. Initialize structural window
        popup_win = Adw.Window()
        popup_win.set_title(title)
        popup_win.set_default_size(width, height)

        # 2. Bind to main window layer
        if parent_window:
            popup_win.set_transient_for(parent_window)
            popup_win.set_modal(True)

        # 3. Handle responsive layouts and overflow
        scroller = Gtk.ScrolledWindow()
        scroller.set_child(content_widget)
        scroller.set_propagate_natural_width(True)
        scroller.set_propagate_natural_height(True)

        # 4. Bind structural layouts together
        toolbar_view = Adw.ToolbarView()
        header_bar = Adw.HeaderBar()

        toolbar_view.add_top_bar(header_bar)
        toolbar_view.set_content(scroller)
        popup_win.set_content(toolbar_view)
    ...
    @classmethod
    def swap_content(cls, current_widget, new_content_widget):
        """
        Traverses upward from any child widget to find the Adw.Window root,
        extracts its inner Adw.ToolbarView, and swaps out the display canvas.
        """
        # 1. Grab the top-level Adw.Window container holding this child widget
        root_window = current_widget.get_root()
        if not root_window or not isinstance(root_window, Adw.Window):
            print("[POPUP SYSTEM] Error: Could not resolve parent Adw.Window layer.")
            return

        # 2. Extract the Adw.ToolbarView we set inside popup_win.set_content()
        toolbar_view = root_window.get_content()
        if not isinstance(toolbar_view, Adw.ToolbarView):
            print("[POPUP SYSTEM] Error: Window root content structure is mismatching.")
            return

        # 3. Handle responsive layouts and overflow wrap for the new view layout
        new_scroller = Gtk.ScrolledWindow()
        new_scroller.set_child(new_content_widget)
        new_scroller.set_propagate_natural_width(True)
        new_scroller.set_propagate_natural_height(True)

        # 4. Swap content frames cleanly and push the visual view update
        toolbar_view.set_content(new_scroller)
        print(f"[POPUP SYSTEM] Successfully transitioned view canvas layer.")
...
class DialogMixin:
    ...
    def show_gnome_setup_dialog(self, button=None):
        ...
        self.gnome_dialog = Adw.MessageDialog(
            transient_for=self, heading="GNOME Shell Refresh Setup"
        )
        ...
        if not portal_path:
            ...
            if portal_path:
                setattr(
                    self, f"active_portal_{self.get_safe_key(host_path)}", portal_path
                )
        ...
        status_expander = Adw.ExpanderRow(
            title="System Status Check", subtitle="Expand to see setup details"
        )
        ...
        def create_status_row(title, is_ready):
            ...
            icon = Gtk.Image.new_from_icon_name(
                "emblem-ok-symbolic" if is_ready else "window-close-symbolic"
            )
        ...
        status_expander.add_row(
            create_status_row(f"Portal Access: {display_path}", has_access)
        )
        status_expander.add_row(
            create_status_row("Systemd .path file found", path_exists)
        )
        status_expander.add_row(
            create_status_row("Systemd .service file found", service_exists)
        )
        ...
        main_box.append(
            Gtk.Label(label=instruction_text, use_markup=True, xalign=0, wrap=True)
        )
        ...
        path_display = Gtk.Label(
            label=host_path, xalign=0, hexpand=True, selectable=True
        )
        ...
        copy_path_btn.connect(
            "clicked",
            lambda b: self.toast_overlay.add_toast(Adw.Toast.new("Path copied!")),
        )
        ...
        # WARNING EXPANDER
        warning_expander = Adw.ExpanderRow(
            title="WARNING", subtitle="Security information"
        )
        ...
        warning_label = Gtk.Label(
            label=warning_text, use_markup=True, xalign=0, wrap=True
        )
        ...
        cmd_label = Gtk.Label(
            label=full_cmd, selectable=True, wrap=True, hexpand=True, xalign=0
        )
        cmd_label.add_css_class("card")

        copy_cmd_btn = Gtk.Button.new_from_icon_name("edit-copy-symbolic")
        copy_cmd_btn.add_css_class("flat")
        copy_cmd_btn.connect(
            "clicked",
            lambda b: (
                Gdk.Display.get_default()
                .get_clipboard()
                .set_content(Gdk.ContentProvider.new_for_value(full_cmd))
            ),
        )
        copy_cmd_btn.connect(
            "clicked",
            lambda b: self.toast_overlay.add_toast(Adw.Toast.new("Command copied!")),
        )
    ...
    def show_plasma_setup_dialog(self, button=None):
        ...
        self.plasma_dialog = Adw.MessageDialog(
            transient_for=self, heading="KDE Plasma Refresh Setup"
        )
        ...
        if has_access:
            path_exists = os.path.isfile(
                os.path.join(portal_path, "plasma-refresher.path")
            )
            service_exists = os.path.isfile(
        ...
        cmd_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
        cmd_box.append(
            Gtk.Label(label="<b>Required Setup Command:</b>", use_markup=True, xalign=0)
        )
        ...
        path_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)
        path_box.add_css_class(
            "card"
        )  # Optional: adds a nice background frame if your CSS supports it
        path_box.set_margin_start(12)

        # Display the path in a selectable label
        path_display = Gtk.Label(
            label=path_to_copy, xalign=0, hexpand=True, selectable=True
        )
        path_display.add_css_class(
            "dim-label"
        )  # Makes it look like a secondary technical path
    ...
    def show_permission_dialog(self, title, folders):
        ...
        self.portal_widgets = {}
        app_id = "temp"  # self.get_application().get_application_id()
        dialog = Adw.MessageDialog(
            transient_for=self, heading=f"Permissions for {title} Required"
        )
        ...
        if title == "Papirus":
            ...
            papirus_command_label = Gtk.Label(
                label=f"{command_papirus}", selectable=True, use_markup=True, wrap=True
            )
        ...
        instruction_label = Gtk.Label(label=msg, use_markup=True, wrap=True, xalign=0)
        ...
        for i, folder in enumerate(folders):
            ...
            short_folder = folder.replace("~/.local/share/", "xdg-data/").replace(
                "~/.config/", "xdg-config/"
            )
            ...
            if title == "Vesktop":
                ...
                header_label = Gtk.Label(
                    label=f"<b>Required Path {i + 1} (Select either):</b>",
                    use_markup=True,
                    xalign=0,
                )
                ...
                def create_path_row(display_name, actual_path):
                    ...
                    type_label = Gtk.Label(
                        label=f"<b>{display_name}:</b>", use_markup=True
                    )

                    # Path label
                    p_label = Gtk.Label(
                        label=actual_path, selectable=True, xalign=0, hexpand=True
                    )
                    p_label.add_css_class(
                        "dim-label"
                    )  # Make it look like a secondary path
                    ...
                    c_btn.connect(
                        "clicked",
                        lambda b, p=actual_path: b.get_clipboard().set_content(
                ...
                # Add the Regular Row
                section_box.append(
                    create_path_row("Regular", "~/.config/vesktop/themes")
                )
                ...
                label = Gtk.Label(
                    label=f"<b>Required Path {i + 1}:</b>", use_markup=True, xalign=0
                )
                section_box.append(label)

                box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
                path_label = Gtk.Label(
                    label=folder, selectable=True, xalign=0, hexpand=True
                )
                ...
                copy_btn.connect(
                    "clicked",
                    lambda b, f=folder: b.get_clipboard().set_content(
            ...
            selected_label = Gtk.Label(
                label=f"<b>Selected Path {i + 1}:</b>", use_markup=True, xalign=0
            )
            ...
            select_btn.connect(
                "clicked",
                lambda b, f=folder: self.on_dialog_response(dialog, "select", f),
            )
            section_box.append(select_btn)

            #  Copy Command
            labelcopy = Gtk.Label(
                label=f"Or use this command for permanent access",
                use_markup=True,
                xalign=0,
            )
            command_text = (
                f"flatpak override --user {app_id} --filesystem={short_folder}:create"
            )
            command_label = Gtk.Label(
                label=f"{command_text}", selectable=True, use_markup=True, wrap=True
            )
    ...
    def zen_permission_dialog(self, title, folders):
        ...
        info_label = Gtk.Label(
            label=instruction_text, use_markup=True, wrap=True, xalign=0
        )
        ...
        for i, folder in enumerate(folders):
            ...
            user_path_entry.set_placeholder_text(
                "Paste your /.zen/profile/chrome path here..."
            )
            ...
            user_path_entry.connect(
                "changed", update_ui, selected_label, command_label, folder
            )
            ...
            section_box.append(
                Gtk.Label(
                    label="<b>Initial setup: Paste your unique Chrome path</b>",
                    use_markup=True,
                    xalign=0,
                )
            )
            ...
            if existing_path:
                ...
                section_box.append(
                    Gtk.Label(
                        label="<b>Step 2: Select your unique Chrome path</b>",
                        use_markup=True,
                        xalign=0,
                    )
                )
            ...
            section_box.append(
                Gtk.Label(
                    label="<b>Or Paste the command in your terminal for permanent access</b>",
                    use_markup=True,
                    xalign=0,
                )
            )
